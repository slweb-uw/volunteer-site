...
//Role entry type
type RoleEntry = { type: string; qty: number };
...
const SignupEventPopup = ({
  ...
  const [selectedDates, setSelectedDates] = useState<string[]>([]); // Array of date strings "YYYY-MM-DD"

  // Calendar Navigation State
  const [currentDateView, setCurrentDateView] = useState(new Date());

  // Volunteers
  const [volunteerData, setVolunteerData] = useState<RoleEntry[]>([]);
  ...
  useEffect(() => {
    if (event && (mode === "edit" || mode === "add")) {
      ...
      if (event.dates && Array.isArray(event.dates)) {
        ...
        const formattedDates = event.dates.map(
      ...
      if (event.openings && Object.keys(event.openings).length > 0) {
        let rolesSource: Record<string, number> = {};
        const keys = Object.keys(event.openings);

        if (
          typeof event.openings[keys[0]] === "object" &&
          event.openings[keys[0]] !== null
        ) {
  ...
  const handleAddRole = () => {
    ...
    if (
      volunteerData.some(
        (v) => v.type.toLowerCase() === newRoleName.toLowerCase(),
      )
    ) {
  ...
  const handleSubmit = () => {
    ...
    if (!leadEmail) {
      alert("Please enter a Lead Contact Email");
      return;
    }
    ...
    const openings = volunteerData.reduce<Record<string, number>>(
      (acc, item) => {
    ...
    const finalEventData = {
      ...
      // Popup builds a flat draft (flat openings + JS Dates); EventAdmin reshapes
      // it into the nested-openings / Timestamp EventData shape on save.
  ...
  const monthName = currentDateView.toLocaleString("default", {
    month: "long",
  });
      ...
      <DialogContent className={classes.content}>
        ...
        <Grid container spacing={4} style={{ marginTop: "16px" }}>
          ...
          <Grid item xs={12} md={4}>
            ...
            <div className={classes.calendarContainer}>
              ...
              <div className={classes.calendarGrid}>
                {daysOfWeek.map((d) => (
                  <div key={d} className={classes.dayLabel}>
                    {d}
                  </div>
                ))}
                {renderCalendar()}
          ...
          <Grid item xs={12} md={3}>
            ...
            >
              Selected Date(s) &nbsp;{" "}
              <span style={{ fontSize: "0.8rem", fontWeight: 400 }}>on</span>
            </Typography>
